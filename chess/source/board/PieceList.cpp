...
bitBoard_t PieceList::getAttackingBB() {
	bitBoard_t attacksBB = 0;
	for (iterator_t it = pieces.begin(); it != pieces.end(); it++) {
			attacksBB |= it->second.canMoveToBB | it->second.pawnAttacksBB; //for non pawns pawnAttacksBB is 0
	}
	return attacksBB
}
...
const_iterator_t PieceList::begin() {
	return pieces.begin();
}
const_iterator_t PieceList::end() {
	return pieces.end();
}
